...
export function ThemeToggle({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <button
      className={compact ? "btn-theme-toggle btn-theme-toggle--compact" : "btn-theme-toggle"}
      onClick={() => setDark((v) => !v)}
      title={dark ? "Switch to light mode" : "Switch to dark mode"}
      aria-label={dark ? "Switch to light mode" : "Switch to dark mode"}
    >
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" width={size} height={size} fill="currentColor">
...
export default function App() {
  ...
  useEffect(() => {
    if (route.kind === "doc") document.title = `pp | ${route.slug}`;
    else if (route.kind === "tree") document.title = "pp | tree";
    else document.title = "plan-present";
  }, [route]);

  if (route.kind === "doc") return <Editor slug={route.slug} />;
  if (route.kind === "tree") return <TreeHome />;
  return (
    <>
      <ThemeToggle />
      <main>
